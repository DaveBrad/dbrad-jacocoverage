...
/**
 * Base annotation class that contains some basic logic and holds information
 * about all annotations for all classes. Based on the work of Andrey Korostelev
 * (original author of "Code Coverage Plugin" based on EMMA for NetBeans 6). See
 * http://plugins.netbeans.org/plugin/5620/unit-tests-code-coverage-plugin for
 * the NetBeans 6.0 EMMA based plugin. See
 * https://blogs.oracle.com/geertjan/entry/creating_error_annotations_in_netbeans2
 * for annotation tips.
 *
 * @author Andrey Korostelev (pre-2017)
...
public abstract class AbstractCoverageAnnotation extends Annotation implements PropertyChangeListener {
    ...
    /** A list of all registered living annotation. Used to know and clear
     * annotations associated to a project. */
    private final static HashMap<String, HashMap<Integer, Annotation>> annotations
            = new HashMap<>(8);

    /** Utility character. Projects are identified by the concatenation of their
     * path, this utility character and the project's name. */
    private final static char KEY_JOIN_CHAR = ' ';

    /**
     * Creates new annotation and adds it to the global annotations list so it
     * can be managed later
     *
     * @param projectName   the project this annotation belongs to.
     * @param classFullName the class (package + name) this annotation belongs
     *                      to.
    ...
    /**
     * Removes all editor annotations from specified project. Currently uses
     * slow string comparing approach to select annotations maps for current
     * project. Might be changed to use more complicated storage structure, but
     * it will make annotations storing more difficult.
     *
     ...
     *
     * @param projectName   the project to remove annotations for.
     * @param classFullName the class (package + name) to remove annotations
     *                      for.
     */
    ...
    public String getAnnotationType() {
        return GlobalsOfConfig.THEME_PREFIX.get(theme);
    }

    /**
     * This method is used to create a unique key for every new class
     * annotation.
     *
     * @param projectName   the project this annotation belongs to.
     * @param classFullName the class (package + name) this annotation belongs
